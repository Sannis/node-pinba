...
PinbaRequest.prototype.getInfo = function () {
  ...
  // Request info
  var info = {
    mem_peak_usage: memory_usage.rss, // We have no exact information
    req_time:       req_time,
  ...
  for (var timer in this.timers) {
    if (this.timers.hasOwnProperty(timer)) {
      info.timers.push(this.timerGetInfo(timer));
    }
  }
...
PinbaRequest.prototype.flush = function (callback) {
  ...
  socket.send(message, 0, message.length, this.pinba_port, this.pinba_host, function (err, bytes) {
    socket.close();

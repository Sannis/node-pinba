...
var PinbaDescriptor = require("../resources/pinba.json");
var ProtoBufPinbaRoot = ProtoBuf.Root.fromJSON(PinbaDescriptor);
var ProtoBufPinbaRequest = ProtoBufPinbaRoot.lookup('Pinba.Request');
/** internal
 * getProtoBufPinbaRequest()
 **/
exports.getProtoBufPinbaRequest = function getProtoBufPinbaRequest () {
  return ProtoBufPinbaRequest;
};
...
// timersGet() flags
exports.GET_ONLY_STOPPED_TIMERS = exports.FLUSH_ONLY_STOPPED_TIMERS;
...
var Request = exports.Request = function (properties) {
  ...
  // Or you can set it manually using setRequestTime
 ...
 *
 * Setter for *request_time* property.
 **/
Request.prototype.setRequestTime = function (request_time) {
...
Request.prototype.timerStart = function (tags, data) {
  ...
  var timer_id = (++this.timerIdGenerator).toString();
 ...
 * Get timers.
 *
 * If flag == Pinba.GET_ONLY_STOPPED_TIMERS, then returns only stopped timers.
 **/
Request.prototype.timersGet = function (flag) {
  var timers = [];

  _.forOwn(this.timers, function (timer, timer_id) {
    if ((flag === exports.GET_ONLY_STOPPED_TIMERS) && timer.started) {
      return;
    }
...
Request.prototype.getInfo = function () {
  ...
  if (this.request_time) {
    ...
    request_time = process.hrtime(this.start);
    request_time = request_time[0] + request_time[1] / 1e9;
  }
  ...
  var self = this;
  _.forOwn(this.timers, function (timer, timer_id) {
    info.timers.push(self.timerGetInfo(timer_id));
  });
...
Request.prototype.flush = function (config, callback) {
  ...
  if (!(flags & exports.FLUSH_ONLY_STOPPED_TIMERS)) {
    this.timersStop();
  }
  ...
  if (flags & exports.FLUSH_RESET_DATA) {
    // Request start time
    this.start = process.hrtime();
  }
  ...
  var buffer;
  try {
    var err = ProtoBufPinbaRequest.verify(data);
    if (err) {
        throw Error(err);
    }

    var message = ProtoBufPinbaRequest.fromObject(data);

    buffer = ProtoBufPinbaRequest.encode(message).finish();
  } catch (err) {
  ...
  // Send message
  var socket = require('dgram').createSocket({type: 'udp4', lookup: dnsCache.lookup});
  socket.on('error', function() {/* do nothing */});
  socket.send(buffer, 0, buffer.length, this.pinba_port, this.pinba_server, function (err /*, bytes*/) {
    socket.close();
